 ...
 **/
const paramValidate=[ check('first_name').not().isEmpty() && check('first_name').isLength({min:2,max:50}).isAlpha().matches(/^[A-Z]+$/i),                      
                ...
                //Function to insert the same tokens to the Doctor's Session
                 updateDoctorSession=()=>{
                  //   console.log("updateDoctorSession");
                     db.collection('DoctorSession').insertOne(session_obj,(err,result1)=>{
                        if(err){
